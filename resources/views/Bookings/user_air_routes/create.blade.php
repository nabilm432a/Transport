    ...
    <div style="display: flex; justify-content: center; align-items: center; padding-bottom: .5rem;">
        <div class="text-white" style="width: 40%;margin-top: 100px; border-radius: 8px; height: 90%; padding: 3rem;background-color: rgb(17, 24, 39);">
            <div style="margin-bottom: 10px; background-color: rgb(45, 55, 72); border-radius: 8px">
                <h2 class="mb-2 ml-2 mt-0 text-2xl font-medium leading-tight text-primary">Transport Routes Over the Air</h2>
            </div>

            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center;">
                <form method="POST" action="{{route('user_air_routes.store')}}">
                    @csrf
                        ...
                        <div class="md:w-2/3">
                            <select class="text-black" name="source_id" id="source_id">
                                @foreach($airports as $airport)
                                    <option class="text-black" value="{{$airport->id}}">{{$airport->name}}, {{$airport->location->city}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <div>
                <a href="{{route('air_routes.index')}}"><button class="mb-3 flex w-full items-center justify-center rounded bg-red-400 bg-primary px-7 pb-2.5 pt-3 text-center text-sm font-medium uppercase leading-normal text-white opacity-100 transition duration-300 ease-in-out hover:opacity-50" data-te-ripple-init data-te-ripple-color="light">Back</button></a>
            </div>
        </div>
    </div>
@endsection
