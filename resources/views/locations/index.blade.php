...
@section('main')
    ...
    <div style="margin-top: 5px">
        <a href="{{route('locations.create')}}"><button class="mb-3 flex w-full items-center justify-center rounded bg-red-400 bg-primary px-7 pb-2.5 pt-3 text-center text-sm font-medium uppercase leading-normal text-white opacity-100 transition duration-300 ease-in-out hover:opacity-50" data-te-ripple-init data-te-ripple-color="light">Add</button></a>
    </div>
    <div style="margin-top: 5px">
            ...
            @foreach($locations as $location)
                <tr class="bg-gray-100">
                    ...
                    <td style="text-align: center;">
                        <a href="{{route('locations.edit', $location)}}" style="color: white;background-color: #9ca3af; padding: 0.4rem">Edit</a>
                        <form style="display: inline-block;color: white; background-color: #9ca3af; padding: 0.3rem" method="POST" action="{{route('locations.destroy', $location)}}">
                            @csrf
                            @method('DELETE')
